...
class PostHelpers {
	...
	static function formatContent( $content )
	{
		...
		$content =  preg_replace_callback('#\[sc:.*\](.+?)\[\/sc\]#s', function($m) use($x) {
	        $sortcode = self::sortcode($m[0] );
        	if(count($sortcode))
        	{
        		$code = ucwords($sortcode['code']).'Helpers|'.$sortcode['params']['fnc'].'|'.$sortcode['params']['id'];
        		return  self::__fnc($code);
        	}
