    		...
    		<div class="infobox infobox-danger fade in">
			  <button type="button" class="close" data-dismiss="alert"> x </button>  
			   ...
			   <p> <i class="fa fa-warning text-danger"></i> <b class="text-danger">Becarefull !</b> while editing Core Table default from Sximo 5 Ultimate distribution </p>	
			</div>	
...
{!! Form::open(array('url'=>$action, 'class'=>'form-horizontal','id'=>'saveform' , 'parsley-validate'=>'','novalidate'=>' ')) !!}
				...
				@foreach($columns as $column)
					<tr class="" id="field-{{ $column->Field }}">
						...
						<td>
							...
							<a href="{{ url('root/database?task=editfield&table='.$table.'&'.$info) }}" class="btn btn-xs btn-primary" onclick="SximoModal(this.href,'Edit New Column : {{$column->Field}} '); return false;" ><i class="fa fa-edit "></i></a>
							<a href="{{ url('root/database?task=removefield&table='.$table.'&field='.$column->Field) }}" rel="#field-{{ $column->Field }}" class="removedField  btn btn-xs btn-danger"><i class="fa fa-minus"></i></a>
						</td>
		...
		@else
		<a href="{{ url('root/database?task=editfield&table='.$table) }}" class="btn btn-xs btn-info" onclick="SximoModal(this.href,'Add New Column'); return false;" ><i class="fa fa-plus"></i> Add New Field</a>
		@endif	
...
function showResponse(data)  {		
	...
	{
		window.location.href = '{{ url("root/database") }}';
